...
function plot_track(jstr)
{
    ...
    for (row in track)
    {
        ...
        if (count == 1) { map.setCenter(new GLatLng(lasLat, lasLon), 13); }
    
        gll = new GLatLng(lasLat, lasLon);
        line.push(gll);
        ...
        if (count % 10 == 0)
        {
            ...
            //polyline = new GPolyline(line, sprintf("#%02x%02x%02x",lasAlt,color*32%256,color*128%256), 3, 1);
            polyline = new GPolyline(line, sprintf("#%02x%02x%02x",lasAlt*((color&0x4)>>2) ,lasAlt*((color&0x2)>>1),lasAlt*(color&0x1)), 3, 1);
    ...
    polyline = new GPolyline(line, sprintf("#%02x%02x%02x",lasAlt*(color&0x100>>2) ,lasAlt*(color&0x010>>1),lasAlt*(color&0x1)), 3, 1);
    map.addOverlay(polyline);
    segments.push(polyline);
...
function plot_task(jstr)
{
    ...
    for (row in track)
    {
        ...
        if (count == 1)
        {
            map.setCenter(new GLatLng(lasLat, lasLon), 13);
        }
        ...
        if (count % 10 == 0)
        {
            polyline = new GPolyline(line, "#0000ff", 2, 1);
            map.addOverlay(polyline);
            segments.push(polyline);
            line = Array();
            line.push(gll);
        ...
        pos = new GLatLng(lasLat,lasLon);
        map.addOverlay(new ELabel(pos, cname, "waypoint", new GSize(-40,0), 60));

        sz = GSizeFromMeters(map, pos, crad*2,crad*2);
        map.addOverlay(new EInsert(pos, "circle.png", sz, map.getZoom()));
...
function plot_award_task(jstr)
{
    ...
    //map.addControl(ovlay, new GControlPosition(G_ANCHOR_BOTTOM_RIGHT, new GSize(128, 256)));
...
function plot_glider(glider,lat,lon)
{
    // FIX: plot a point ...
    var para;
    var mark;
    ...
    para.iconAnchor = new GPoint(10,20)
    // load image ...
    mark = new GMarker(new GLatLng(lat,lon),{ icon: para });
    map.addOverlay(mark);
    if (onscreen[glider]["icon"])
    {
        map.removeOverlay(onscreen[glider]["icon"]);
    }
    ...
    for (glider in onscreen)
    {
        ...
        if (count < track.length)
        {
            ...
            if (lasLat != 0)
            {
                ...
                plot_glider(glider,lasLat,lasLon);
            }
        }
    }
