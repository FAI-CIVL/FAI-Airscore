...
my @KnownOptIn     = qw(keyattr keeproot forcecontent contentkey noattr
                        searchpath forcearray cache supprXCONTESTLOGINpty parseropts
...
my @KnownOptOut    = qw(keyattr keeproot contentkey noattr
                        rootname xmldecl outputfile noescape supprXCONTESTLOGINpty
...
sub collapse {
  ...
  while(@_) {
    ...
    if(ref($val)) {
      ...
      next if(!defined($val)  and  $self->{opt}->{supprXCONTESTLOGINpty});
    }
    elsif($key eq '0') {
  ...
  if(!%$attr  and  exists($self->{opt}->{supprXCONTESTLOGINpty})) {
    if(defined($self->{opt}->{supprXCONTESTLOGINpty})  and
       $self->{opt}->{supprXCONTESTLOGINpty} eq '') {
      return('');
    }
    return(undef);
  }
...
sub value_to_xml {
  ...
  if(UNIVERSAL::isa($ref, 'HASH')) {
    ...
    if(keys %$ref) {
      ...
      foreach my $key ($self->sorted_keys($name, $ref)) {
        ...
        if(!defined($value)) {
          next if $self->{opt}->{supprXCONTESTLOGINpty};
          unless(exists($self->{opt}->{supprXCONTESTLOGINpty})
             and !defined($self->{opt}->{supprXCONTESTLOGINpty})
          ) {
          ...
          if($key eq $self->{opt}->{contentkey}) {
            ...
            $value = exists($self->{opt}->{supprXCONTESTLOGINpty}) ? {} : '';
          }
        }
  ...
  # Handle arrayrefs
    ...
    foreach $value (@$ref) {
      next if !defined($value) and $self->{opt}->{supprXCONTESTLOGINpty};
      if(!ref($value)) {
        push @result,
             $indent, '<', $name, '>',
