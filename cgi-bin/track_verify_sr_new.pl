...
sub validate_task
{
    ...
    my $stopalt = 0;				# altitude when task was stopped
    my $stoptime = 0;				# Time task was stopped
    my $mystarttime = undef;		# Time I made start
    my $goaltime = undef;			# Time I made goal (ESS?)
    my $startss = 0;				# Time Start opens
    my $endss = undef;				# Time Start closes
    my $lastin = -1;
    ...
    my ($spt, 						# Start type (entry, exit)
    	$ept, 						# ESS type (entry, exit)
    	$gpt, 						# Goal type (entry, exit, line)
    	$ssdist, 					# Speed Section length
    	$launchssdist, 				# Distance to start
    	$endssdist, 				# Distance to ESS
    	$totdist) 					# Total task distance
    	= task_distance($task);
    ...
    for $coord (@$coords)
    {
        ...
        if (defined($task->{'sstopped'}) && !defined($endss)) 
        {
            ...
            if (($coord->{'time'} > $task->{'sstopped'}) or
                (defined($maxtime) && ($coord->{'time'} > $mystarttime + $maxtime)))
            {
                print "Coordinate after stopped time: ", $coord->{'time'}, " ", $task->{'sstopped'}, ".\n";
                # Pilot position and altitude at stopped time
                $stopalt = $coord->{'alt'};
        ...
        # Check if we did re-enter and set the task "back"
        if (($lastin == $spt)
                and ((($task->{'type'} eq 'race') and ($mystarttime < $task->{'sstart'})) or ($task->{'type'} ne 'race')))
        {
            $rpt = $waypoints->[$lastin];
            # Re-entered start cyclinder?
            $rdist = distance($coord, $rpt);
            if ( $rpt->{'how'} eq 'entry' )
            {
                ...
                if ( ($rdist < ($rpt->{'radius'} - $rpt->{'margin'})) and ($reflag == $lastin) )
                {
                    ...
                    if (($task->{'type'} eq 'race') && ($coord->{'time'} > $task->{'sstart'}))
                    {
                        $mystarttime = 0 + $task->{'sstart'};		# startgate time
                    }
                    elsif (($task->{'type'} eq 'speedrun-interval') && ($mystarttime > $taskss))
                    {
                        $mystarttime = 0 + $taskss + floor(($coord->{'time'}-$taskss)/$interval)*$interval;		#last startgate time before my time
                    }
                    else
                    {
                    	$mystarttime = 0 + $coord->{'time'};		# my time
                    }
            ...
            if ( $rpt->{'how'} eq 'exit' ) 
            {
                ...
                elsif ($rdist >= ($rpt->{'radius'} + $rpt->{'margin'}) and ($reflag == -1))
                {
                    ...
                    if (($task->{'type'} eq 'race') && ($coord->{'time'} > $task->{'sstart'}))
                    {
                        $mystarttime = 0 + $task->{'sstart'};
                    }
                    elsif (($task->{'type'} eq 'speedrun-interval') && ($mystarttime > $taskss))
                    {
                        $mystarttime = 0 + $taskss + floor(($coord->{'time'}-$taskss)/$interval)*$interval;
                    }
                    else
                    {
                    	$mystarttime = 0 + $coord->{'time'};
                    }
                    $startss = $mystarttime;
                    $coeff = 0; $coeff2 = 0; 
                    if ($debug)
        ...
        #print "wcount=$wcount wmade=$wmade newdist=$newdist maxdist=$maxdist starttime=$mystarttime time=", $coord->{'time'}, "\n";

        # Work out leadout coeff / maxdist if we've moved on
        # bestDistToESS(tp0) = LengthOfSpeedSection
        # bestDistToESS(tpi) = min(bestDistToESS(tpi−1), LenghtOfSpeedSection − distanceFlown(tpi))
        $taskTime = $coord->{'time'} - $startss;
        $bestDistToESS = ($ssdist - $maxdist)/1000;
        my $newDistToESS = ($ssdist - $newdist)/1000;
        if ( defined($mystarttime) and ($newdist > $maxdist) )
        {
            if (!defined($endss))
            {
                if (defined($lastmaxcoord) && !defined($endss))
                {
                    $coeff = $coeff + $taskTime * ( ($ssdist - $maxdist) - ($ssdist - $newdist) ); # this is = coeff + tt * (newdist - maxdist) : is it what is meant to be?
                    #$coeff2 = $coeff2 + ($coord->{'time'} - $startss) * ( ($ssdist - $maxdist)*($ssdist - $maxdist) - ($ssdist - $newdist)*($ssdist - $newdist) );
                    # distances should be in km
		     		$coeff2 = $coeff2 + $taskTime * ( $bestDistToESS**2 - $newDistToESS**2 );
                }
                $lastmaxcoord = $coord;
                if ($debug)
                {
                    print "newdist=$newdist maxdist=$maxdist time=", ($coord->{'time'} - $startss), " distrem=", ($ssdist - $maxdist), " ncoeff=$coeff\n";
                }
            }

            $maxdist = $newdist;
            if (($maxdist >= $launchssdist) && (!defined($endss)) 
                && ($kmtime->[floor(($maxdist-$launchssdist)/1000)] == 0))
            {
                $kmtime->[floor(($maxdist-$launchssdist)/1000)] = $coord->{'time'};
                print "kmtime ($maxdist): ", floor(($maxdist-$launchssdist)/1000), ":", $coord->{'time'}, "\n";
            }
            # else { print "new max ($maxdist)\n"; }
        ...
        if ($wpt->{'how'} eq 'entry')
        {
            if (made_entry_waypoint($waypoints, $wmade, $coord, $dist, $awarded))
            {
                ...
                # Do task timing stuff
                if (($wpt->{'type'} eq 'launch') and (!defined($mystarttime)) or
                    ($wpt->{'type'} eq 'speed'))
                {
                    # get last start time ..
                    # and in case they were too lazy to put in startss ...
                    $mystarttime = 0 + $coord->{'time'};
                    $preSScoord = $lastcoord;
                    if (($task->{'type'} eq 'race') && ($mystarttime > $task->{'sstart'}))
                    {
                        $mystarttime = 0 + $task->{'sstart'};
                    }
                    if ($awarded == 1 && $awtime > 0)
    ...
    #   starttime -  actual start time
    #   startss - start of task time (back to gate)
    print "wasinSS=$wasinSS taskss=$taskss startss=$startss starttime=$mystarttime interval=$interval\n";
    ...
    if ($task->{'type'} eq 'speedrun-interval')
    {
        ...
        if ($startss > $taskss) 
        {
            $startss = 0 + $taskss + floor(($mystarttime-$taskss)/$interval)*$interval;
        }
    }
    ...
    # Jumped the start/speedss?
    # Early start in Hang-gliding (Jump The Gun feature)
    # A maximum early start of X seconds is allowed, as it is considered as a safety feature in potential crowded environment.
    # Early start is penalized in FS by a factor of Y points per second of early start time. 
    # By default, the penalty is 2 points for every second, up to a maximum of 300 seconds. 
    # Early start of more than X seconds is scored as minimum distance. 
    # The penalty factor and maximum time may be adjusted for local conditions and must be stated in the local regulations if they differ from the default values.
    
    my $maxjump = 300;
    
    if ( ($task->{'class'} ne 'PG') and ($task->{'type'} ne 'route') and defined($mystarttime) and (($mystarttime < $startss) or ($mystarttime < $taskss)) and ($wmade > $spt))
    {
        my $jump = $taskss - $startss;
        print "Jumped the start gate ($spt) (taskss=$taskss finish=$finish) (startss=$startss: $mystarttime)\n";

        if ( ($jump <= $maxjump) and $task->{'type'} eq 'race' )
		{
			$comment = "jumped $jump secs";
			# clear leadout markers
			$kmtime = init_kmtime($task->{'ssdistance'});
			...
			# shift leadout graph so it doesn't screw other lo points
			# Should be covered by using starttime elsewhere now ..
			#$coeff = $coeff + $task->{'ssdistance'}*($startss-$mystarttime);
		}		
		else
		{
			# Otherwise it's a zero for elapsed (?)
			$coeff = $coeff + $ssdist*($startss-$taskss);
			$coeff2 = $coeff2 + ($ssdist/1000)**2 * ($startss-$taskss);
			if ($waypoints->[$spt]->{'how'} eq 'entry')
			{
				print "Elasped entry jump: $comment\n";
				$wcount = $spt - 1;
    ...
    if ($wmade == 0)
    {
        ...
        print "wcount=0 dist=$dist_flown\n";
        #$coeff = $coeff + ($ssdist - $dist_flown)*($task->{'sfinish'}-$startss);
        $coeff = $ssdist * ($task->{'sfinish'}-$task->{'sstart'});
        $coeff2 = ($ssdist/1000)**2 * ($task->{'sfinish'}-$task->{'sstart'}); # Did not takeoff? Did fly but to T/O wpt?
    }
    elsif ($wcount == 0)
    {
        print "Didn't make startss ($maxdist), closest wpt=$closestwpt\n";
        ...
        print "wcount=0 dist=$dist_flown\n";
        $coeff = $ssdist * ($task->{'sfinish'}-$task->{'sstart'});
        $coeff2 = ($ssdist/1000)**2 * ($task->{'sfinish'}-$task->{'sstart'});
    }
    elsif ($wcount < $allpoints)
    {
        # we didn't make it into goal
        ...
        $dist_flown = $maxdist; # distance_flown($waypoints, $wmade, $closestcoord);
        $remainingss = $ssdist - $dist_flown + $launchssdist;
        ...
        if (!defined($endss))
        {
            $coeff = $coeff + $ssdist * ($startss - $taskss) + $remainingss * ($task->{'sfinish'}-$coord->{'time'});
            $coeff2 = $coeff2 +  ($remainingss/1000)**2 *  ($task->{'sfinish'} - $startss ); # we need to do this after other operations
        }
    }
    else
    ...
    print "## coeff=$coeff essdist=$ssdist\n";
    $result{'coeff'} = $coeff / 1800 / $ssdist;
    
	$result{'coeff2'} = $coeff2 / (1800 * ($ssdist/1000)**2);
