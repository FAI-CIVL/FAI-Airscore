...
class Reader:
    ...
    def read(self, fp):
        ...
        #print("time: {} ".format(len(timelist)))
        #print("pres: {} ".format(len(preslist)))
        #print("coord: {} ".format(len(coordlist)))

        """create datetime obj from string"""
        #print ("Prova funzione data: {} | {}".format(datetime.datetime.strptime(start, "%Y-%m-%dT%H:%M:%S").date(), datetime.datetime.strptime(start, "%Y-%m-%dT%H:%M:%S").time()))
        flightdate = datetime.datetime.strptime(start, "%Y-%m-%dT%H:%M:%S")
        time = flightdate.time()
        day = flightdate.date()
        #print ("Date and start time of flight: {} {} \n".format(day, time))

        #starttime = self.decode_time(time)
        #day = self.decode_date(day)
...
#     """
#     A low level reader for the KML flight log file format.
#     """
# 
