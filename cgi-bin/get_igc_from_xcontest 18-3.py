...
def get_xc_parameters(task_id, DB_User, DB_Password, DB):
    ...
    db = pymysql.connect(user = DB_User, passwd = DB_Password, db = DB)
    c = db.cursor()
    c.execute("SELECT rwpXcSite_id, rwpXcTakeoff_id, tasDate FROM tblTaskWaypoint JOIN tblTask ON tblTaskWaypoint.tasPk = tblTask.tasPk JOIN tblRegionWaypoint ON tblRegionWaypoint.rwpPk = tblTaskWaypoint.rwpPk WHERE tblTask.tasPk =%s AND tawType = 'launch'", (task_id,))
...
def main():
    ...
    task_id = 0
    xc_zip_destination = '../xcontest/'
    zip_name = 'igc-from_xc.zip'
    submit_zip_directory = '../xcontest/to_import/'
