                ...
                if obj.has_key('waypoints'):
                    if len(obj['waypoints'])>1:
                        ...
                        for w in obj['waypoints']:
                            table_rows += '<tr><td>'+str(obj['waypoints'].index(w)+1)+'</td><td>'+str(w['description'])+'</td></tr>'
...
def get_task_fromfile(jsontask):
    ...
    for obj in jsontask['turnpoints']:
        table_rows += '<tr><td>'+str(obj['waypoint']['name'])+' | '+str(obj['radius'])+'m </td><td>'+str( '<br>'.join([ str(k)+': '+str(v) for k,v in obj['waypoint'].items()]) )+'</td></tr>'
        turnpoints.append({ 'radius':obj['radius'],
            ...
            if file and allowed_file(file.filename):
                ...
                if 'task' in request.files:
                    ...
                    if task and allowed_file(task.filename):
                        ...
                        task.save(save_task)
                    flash('File IGC & TASK have been uploaded: Save this URL to see the result directly (inlcuding parameters)')
