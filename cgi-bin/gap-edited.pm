...
sub spread
{
    ...
    for my $j ( 1 .. $sz )
    {
        ...
        $nbuc->[$j] = 0.0 + $buc->[$j] * 2 / 3;
        #$nbuc->[$j+1] = $buc->[$j] / 3;
        #print "nbuc j " . $nbuc->[$j] . " + buc j " . $buc->[$j] . "\n";
    }
...
sub pilot_distance
{
    ...
    print $pil->{'traPk'}, " lin dist: ";
    print $Adistance * ($pil->{'distance'}/$taskt->{'maxdist'}) * $formula->{'lineardist'}, "\n";
    print " dif dist: ", $Adistance * $kmdiff->[floor($pil->{'distance'}/100.0)] * (1-$formula->{'lineardist'}), "\n";
...
sub ordered_results
{
    ...
    while ($ref = $sth->fetchrow_hashref()) 
    {
        ...
        push @pilots, \%taskres;
        #print "taskres Array: " . Dumper(@pilots);
    }
