...
#       return pilPk
    ...
    def DB_read(cls, traPk):
        ...
        query="select trlLatDecimal, " \
              "trlLongDecimal, " \
              "trlAltitude, " \
              "trlTime " \
              "from tblTrackLog where traPk=%s order by trlTime"

        with Database() as db:
            # get the formula details.
            t = db.fetchall(query, [traPk])

        for row in t:
            fix = GNSSFix(row['trlTime'], row['trlLatDecimal'], row['trlLongDecimal'], True, 0, row['trlAltitude'], None)
            fix.alt = row['trlAltitude']
