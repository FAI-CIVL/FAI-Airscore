...
def get_xc_parameters(task_id, DB_User, DB_Password, DB):
    ...
    with Database() as db:
        query = ("SELECT rwpXcSite_id, rwpXcTakeoff_id, tasDate FROM tblTaskWaypoint JOIN tblTask ON tblTaskWaypoint.tasPk = tblTask.tasPk JOIN tblRegionWaypoint ON tblRegionWaypoint.rwpPk = tblTaskWaypoint.rwpPk WHERE tblTask.tasPk =%s AND tawType = 'launch'", (task_id,))
