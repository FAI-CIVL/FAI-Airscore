...
class Flight:
    ...
    def _parse_h_record(self, record):
        #if re.match('/(?:HFDTE(?:DATE:)?)(\d{2})(\d{2})(\d{2})/mi', record, flags=re.IGNORECASE):
        date = None
        if record[0:10] == 'HFDTEDATE:':
            date = decode_date(record[10:16])
        elif record[0:5] == 'HFDTE':
            date = decode_date(record[5:11])
        if date:
            # match = re.match('/(?:HFDTE(?:DATE:)?)(\d{2})(\d{2})(\d{2})/mi', record, flags=re.IGNORECASE)
            # if match:
                #dd, mm, yy = map(_strip_non_printable_chars, match.groups())
                # year = int("20%s" % xdate[2])
                # month = int(mm)
                # day = int(dd)
