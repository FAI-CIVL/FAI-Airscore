...
sub distance
{
    ...
    $test = haversine_distance($p1->{'lat'}, $p1->{'long'}, $p2->{'lat'}, $p2->{'long'});
    print "haversine dist was $test :: ";
    $d1->{'lat'} = rad2deg($p1->{'lat'});
    $d1->{'long'} = rad2deg($p1->{'long'});
    $d2->{'lat'} = rad2deg($p2->{'lat'});
    $d2->{'long'} = rad2deg($p2->{'long'});
    print "d1 lat = $d1->{'lat'} , lon = $d1->{'long'} - d2 lat = $d2->{'lat'} , lon = $d2->{'long'}\n";
    $dist = WGS84Distance($d1->{'lat'}, $d1->{'long'}, $d2->{'lat'}, $d2->{'long'});
    print "WGS84 Distance = $dist\n";
    return $dist;
}
