...
$dbh->do('UPDATE tblTrack SET traGlider=?, traDHV=?, traFile=? WHERE traPk=?', undef, $glider, $cert, $igc, $traPk);
...
#     }
    elsif ($tasType eq 'elapsed time' or $tasType eq 'race')
    {
        # Optional really ...
        #`${BINDIR}optimise_flight.pl $traPk $comPk $tasPk 3`;
